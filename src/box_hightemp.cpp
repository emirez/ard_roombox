...
#define  HIGH_TEMP_PORT_A2 1
#define  HIGH_TEMP_PORT_A1 2
...
//    HIGH_TEMP_PORT_A1 = _pinTmp;
//    HIGH_TEMP_PORT_A2    = _HIGH_TEMP_PORT_A2;


void hightemp_s::init()
{
    ...
    delay(10);
    Serial.print("HIGH_TEMP_PORT_A1 = ");Serial.println(HIGH_TEMP_PORT_A1);

    delay(10);
    Serial.print("HIGH_TEMP_PORT_A2 = ");Serial.println(HIGH_TEMP_PORT_A2);
}

float hightemp_s::getThmc()
{
...
int hightemp_s::getAnalog(int pin)
{
    long sum = 0;
...
float hightemp_s::getRoomTmp()
{
    int a = getAnalog(HIGH_TEMP_PORT_A1)*50/33;                                // 3.3V supply
...
float hightemp_s::getThmcVol()                                             // get voltage of thmc in mV
{
    float vout = (float)getAnalog(HIGH_TEMP_PORT_A2)/1023.0*5.0*1000;
    float vin  = (vout - VOL_OFFSET)/AMP_AV;
    return (vin);
}


float hightemp_s::K_VtoT(float mV)
{
    int i = 0;
    float value = 0;
